...
const inputFile = "./dbsupermercados-27b.out";
const outputFile = "./dbsupermercados-27b-parsed.out";
const errFile = "./dbsupermercados-27b-error.out";
...
const productList = list.flatMap((e) => {
  if (e.response.constructor == Array)
    return e.response.map((el: any) => ({ ...el, file: e.file }));
...
for (const e of filtered) {
  ...
  const r = {
    ...
    supermarket,
    hash,
    size_weight_count: `${e.size ?? ""} ${e.size_or_count ?? ""} ${
      e.size_weight_count ?? ""
    } ${e.product_size ?? ""} ${e.measurement_unit ?? ""}`
      .replace(/\s+/g, " ")
      .trim(),
